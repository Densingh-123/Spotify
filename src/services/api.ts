...
export const getStreamUrl = async (videoId: string): Promise<string | null> => {
  try {
    ...
    // Deep fallback to piped
    let url = `https://pipedapi.kavin.rocks/streams/${videoId}`;
...
export const getYoutubeId = async (title: string, artist: string): Promise<string | null> => {
  try {
    ...
    let url = `https://pipedapi.kavin.rocks/search?q=${query}&filter=videos`;
    if (APP_PLATFORM === 'web') url = `${CORS_PROXY}${encodeURIComponent(url)}`;
...
export const fetchTrending = async (): Promise<SongItem[]> => {
  try {
    ...
    let url = `https://www.jiosaavn.com/api.php?p=1&q=${query}&_format=json&_marker=0&ctx=wap6dot0&n=40&__call=search.getResults`;
    if (APP_PLATFORM === 'web') url = `${CORS_PROXY}${encodeURIComponent(url)}`;
...
export const searchMusic = async (query: string, offset: number = 0): Promise<SongItem[]> => {
  ...
  try {
    ...
    let url = `https://www.jiosaavn.com/api.php?p=${page}&q=${encodedQuery}&_format=json&_marker=0&ctx=wap6dot0&n=20&__call=search.getResults`;
    if (APP_PLATFORM === 'web') url = `${CORS_PROXY}${encodeURIComponent(url)}`;
...
export const fetchRingtones = async (): Promise<SongItem[]> => {
  try {
    ...
    let url = `https://www.jiosaavn.com/api.php?p=1&q=${query}&_format=json&_marker=0&ctx=wap6dot0&n=30&__call=search.getResults`;
    if (APP_PLATFORM === 'web') url = `${CORS_PROXY}${encodeURIComponent(url)}`;
...
export const searchRingtones = async (query: string): Promise<SongItem[]> => {
  try {
    const encodedQuery = encodeURIComponent(query);
    let url = `https://www.jiosaavn.com/api.php?p=1&q=${encodedQuery}&_format=json&_marker=0&ctx=wap6dot0&n=30&__call=search.getResults`;
    if (APP_PLATFORM === 'web') url = `${CORS_PROXY}${encodeURIComponent(url)}`;
...
export const getLyrics = async (id: string, title?: string, artist?: string, album?: string, duration?: number): Promise<LyricsData> => {
  try {
    ...
    let url = `https://pipedapi.kavin.rocks/lyrics/${ytId}`;
    if (APP_PLATFORM === 'web') {
